...
const Services: React.FC = () => {
  ...
  return (
    <section id="services" className="py-24 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#09090b]">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-b border-zinc-800 pb-8">
          <div>
            <span className="mono text-cyan-500 dark:text-cyan-500 text-xs tracking-widest block mb-2">/// CAPABILITIES</span>
            <h2 className="text-4xl font-bold text-zinc-900 dark:text-white font-['Space_Grotesk'] uppercase">Technical Deployment</h2>
          </div>
          <p className="text-zinc-600 dark:text-zinc-500 max-w-md text-sm mt-4 md:mt-0">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((s) => (
            <div key={s.id} className="tech-card p-8 flex flex-col h-full group">
              ...
              <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-4 font-['Space_Grotesk'] uppercase">{s.title}</h3>
              <p className="text-zinc-600 dark:text-zinc-400 text-sm leading-relaxed mb-8 flex-1">{s.desc}</p>
